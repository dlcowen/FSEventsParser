...
def EnumerateFlags(flag,f_map):
    ...
    for i in f_map:
        if (i & flag):
            if f_map[i]=='FolderEvent;' or f_map[i]=='FileEvent;' or f_map[i]=='SymbolicLink;' or f_map[i]=='HardLink;':
...
class FSEventHandler():
    ...
    def FindDate(self,raw_file):
        ...
        m_regex = "("+regex_1+"|"+regex_2+"|"+regex_3+"|"+regex_4+"|"+regex_5+"|"+regex_6+"|"+regex_7+"|"+regex_8+")"+flag_regex
        ...
        self.time_range = sorted(self.time_range, key=self.getKey)
    ...
    def FindPageRecords(self,page_buf,page_start_off):
        ...
        FindPageRecords will identify all records within a given page.
        '''
        ...
        while len_buf > start_offset and self.valid_record_check==True:
            ...
            self.valid_record_check = self.checkRecord(record.mask,fullpath)
            ...
            # Otherwise assign attributes and add to outpur reports
                ...
                dates = self.applyDate(record.wd)
                ...
                output.Print(self.l_all_fsevents)
                
                fullpath = ''            
        
    def checkRecord(self,mask,fullpath):
        '''
...
class Output(dict):
    ...
    def Print(self,outfile):
        ...
        for i in values:
            valsToInsert+=i.replace('"','""')+'","'
            
        valsToInsert = '"' + valsToInsert[:-3] + '"'
